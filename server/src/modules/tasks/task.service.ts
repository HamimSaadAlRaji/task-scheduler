...
export const getByUserId = (userId: string) =>
  Task.find({ createdBy: userId }).populate("createdBy", "username").exec();

export const create = (data: Partial<TaskDocument>) => new Task(data).save();

export const update = (id: string, data: Partial<TaskDocument>) =>
  Task.findByIdAndUpdate(id, data, { new: true }).exec();
